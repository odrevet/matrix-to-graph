...
void sprite_focus(sprite o_sprite, graph *p_graph, point *v_key)
{
    ...
    char bQuitter = 0;
		char bDeplacement = 0;
		int NbSommets = p_graph->i_nb_summit;
      ...
      if(_rowread(v_key[ENTER].i_y)&v_key[ENTER].i_x){
      			temporize(2500);
						if(bDeplacement==0){		//Enter et sans 'focus sommets' -> chercher sommet...
							do{
								if((o_sprite.coord.i_x>=p_graph->v_summit[i].coord.i_x-p_graph->i_ray)&&
								(o_sprite.coord.i_y>=p_graph->v_summit[i].coord.i_y-p_graph->i_ray)&&
								(o_sprite.coord.i_x<p_graph->v_summit[i].coord.i_x+p_graph->i_ray)&&
								(o_sprite.coord.i_y<p_graph->v_summit[i].coord.i_y+p_graph->i_ray)){
										bDeplacement = 1;
										strcpy(szHelpMessage, "Déplacement de ");
										strcat(szHelpMessage, p_graph->v_summit[i].sz_name);
										ST_helpMsg(szHelpMessage);
								}
								else{
									if(i==NbSommets){
										ST_helpMsg("Aucun sommet trouvé ! ");
										i=0;
									}
									else{
										i++;
						...
						else{									//Enter avec 'focus sommet' -> effectuer deplacement puis focus off
							p_graph->v_summit[i].coord = set_coord(o_sprite.coord.i_x, o_sprite.coord.i_y);
							graph_draw(p_graph);
							sprite_draw(o_sprite);
							bDeplacement = 0;
							ST_helpMsg("Déplacement éffectué. ");
						}
