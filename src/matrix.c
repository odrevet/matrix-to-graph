...
void matrix_malloc(matrix *p_matrix)
{
  ...
  {
    p_matrix->ppi_data[i] = (int *)malloc(p_matrix->i_size * sizeof(int));
  }
}
...
char matrix_product(const matrix *A, const matrix *B, matrix *Z)
{
  ...
  {
    printf("matrix_product error : the matrices must have the same size");
    return 0;
  }
...
char has_next(const matrix *p_matrix, int iLigne)
{
  ...
  for (iColonne = 0; iColonne < p_matrix->i_size; iColonne++)
  { //Pour cette colonne
...
char has_prev(const matrix *p_matrix, int iColonne)
{
  ...
  for (iLigne = 0; iLigne < p_matrix->i_size; iLigne++)
  { //Pour cette ligne
